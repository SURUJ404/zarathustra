...
#[derive(Parser)]
#[grammar = "zarathustra.pest"]
struct ZarathustraParser;

#[allow(clippy::result_large_err)]
pub fn parse(input: &str) -> Result<Pairs<'_, Rule>, Error<Rule>> {
    ZarathustraParser::parse(Rule::file, input)
}
...
mod tests {
                ...
                glob("../zarathustra_cli/examples/**/*.zara").expect("Failed to read glob pattern")
            {
                match entry {
                    Ok(path) => {
                        ...
                        assert!(ZarathustraParser::parse(Rule::file, &data).is_ok());
                    }
                    Err(e) => panic!("{:?}", e),
                }
            }
    ...
    mod rules {
        ...
        fn parse_for_loop() {
            ...
            let parse = ZarathustraParser::parse(Rule::iteration_statement, input);
            assert!(parse.is_ok());
        }
    }
}
