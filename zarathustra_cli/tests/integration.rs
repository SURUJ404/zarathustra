...
mod integration {
    ...
    use tempdir::TempDir;
    use zarathustra_abi::{parse_strict, Encode};
    use zarathustra_ast::ir::Witness;
    ...
    mod helpers {
        use super::*;
        use zarathustra_ast::common::flat::Variable;
    ...
    fn test_compile_and_witness_dir() {
        ...
        let solidity_test_path = global_base.join("zarathustra_verifier");
    ...
    fn test_compile_and_witness(
        ...
        let proving_key_path = tmp_base
            ...
            .join("proving")
            .with_extension("key");
        let solidity_test_path = global_path.join("zarathustra_verifier");
        ...
        let inputs_abi: zarathustra_abi::Inputs<zarathustra_field::Bn128Field> =
            parse_strict(&json_input_str, signature.inputs)
                .map(zarathustra_abi::Inputs::Abi)
                .map_err(|why| why.to_string())
                .unwrap();
        ...
        let expected_witness: Witness<zarathustra_field::Bn128Field> =
        ...
        let witness = Witness::<zarathustra_field::Bn128Field>::read(witness_file).unwrap();

        // load the actual inline witness
        let inline_witness_file = File::open(&inline_witness_path).unwrap();
        let inline_witness =
            Witness::<zarathustra_field::Bn128Field>::read(inline_witness_file).unwrap();
    ...
    fn test_rng_tutorial() {
        ...
        let stdlib = std::fs::canonicalize("../zarathustra_stdlib/stdlib").unwrap();
        let binary_path = env!("CARGO_BIN_EXE_zarathustra");
