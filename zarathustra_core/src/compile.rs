...
use zarathustra_analysis::{self, analyse};
use zarathustra_ast::ir::{self, from_flat::from_flat};
use zarathustra_ast::typed::abi::Abi;
use zarathustra_ast::untyped::{Module, OwnedModuleId, Program};
use zarathustra_ast::zir::ZirProgram;
use zarathustra_codegen::from_program_and_config;
pub use zarathustra_common::{CompileConfig, Resolver};
use zarathustra_field::Field;
use zarathustra_pest_ast as pest;
...
pub enum CompileErrorInner {
    ...
    ReadError(io::Error),
    AnalysisError(zarathustra_analysis::Error),
}
...
impl From<zarathustra_analysis::Error> for CompileErrorInner {
    fn from(error: zarathustra_analysis::Error) -> Self {
...
pub fn compile<'ast, T: Field, E: Into<imports::Error>>(
    ...
    CompileErrors,
> {
    let (typed_ast, abi): (zarathustra_ast::zir::ZirProgram<'_, T>, _) =
