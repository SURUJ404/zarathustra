...
use zarathustra_ast::typed::types::GTupleType;
use zarathustra_ast::typed::ConcreteSignature;
use zarathustra_ast::typed::ConcreteType;
use zarathustra_common::CompileConfig;
use zarathustra_core::compile::compile;

use zarathustra_field::{Bls12_377Field, Bls12_381Field, Bn128Field, Bw6_761Field, Field};
...
type TestResult = Result<Output, zarathustra_interpreter::Error>;
...
) -> Result<Vec<T>, zarathustra_abi::Error> {
    use zarathustra_abi::Encode;
    zarathustra_abi::parse_strict_json(s, types).map(|v| v.encode())
}

fn compare(
    result: Result<Output, zarathustra_interpreter::Error>,
    expected: TestResult,
) -> Result<(), String> {
    if result != expected {
...
pub fn test_inner(test_path: &str) {
    ...
    let t = Tests {
        entry_point: Some(
            t.entry_point
                .unwrap_or_else(|| PathBuf::from(String::from(test_path)).with_extension("zara")),
        ),
        ..t
    };
...
fn compile_and_run<T: Field>(t: Tests) {
    ...
    for test in t.tests.into_iter() {
        ...
        use zarathustra_abi::Decode;

        let output: Result<Output, zarathustra_interpreter::Error> = output.map(|witness| Output {
            value: zarathustra_abi::Value::decode(witness.return_values(), *signature.output.clone())
                .into_serde_json(),
        });
