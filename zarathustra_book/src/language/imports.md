...
You can separate your code into multiple Zarathustra files using `import` statements to import symbols, ignoring the `.zara` extension of the imported file.
...
Absolute imports don't start with `./` or `../` in the path and are used to import components from the Zarathustra standard library. Please check the according [section](../toolbox/stdlib.md) for more details.