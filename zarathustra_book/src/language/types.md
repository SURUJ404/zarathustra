...
Zarathustra currently exposes two primitive types and two complex types:

## Primitive Types

### `field`

This is the most basic type in Zarathustra, and it represents a field element with positive integer values in `[0,  p - 1]` where `p` is a (large) prime number.
...
Booleans are available in Zarathustra. When a boolean is used as a parameter of the main function, the program is constrained to only accept `0` or `1` for that parameter. A boolean can be asserted to be true using an `assert(bool)` statement.
...
Zarathustra provides two complex types: arrays and structs.

### Arrays

Zarathustra supports static arrays, i.e., whose length needs to be known at compile time. For more details on generic array sizes, see [constant generics](../language/generics.md)
...
Zarathustra offers a special shorthand syntax to initialize an array with a constant value:
`[value; repetitions]`
...
#### Spreads and Slices
Zarathustra provides some syntactic sugar to retrieve subsets of arrays.
...
This operation is called slicing, and the following example shows how to slice in Zarathustra:
```
field[3] a = [1, 2, 3];
