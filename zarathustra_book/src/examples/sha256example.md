...
Let's jump into Zarathustra by working through a hands-on project together!
...
In particular, we'll show how Zarathustra and the Ethereum blockchain can be used to allow a prover, let's call her Peggy, to demonstrate beyond any reasonable doubt to a verifier, let's call him Victor, that she knows a hash preimage for a digest chosen by Victor, without revealing what the preimage is.
...
## Computing a Hash using Zarathustra

We will start this tutorial by using Zarathustra to compute the hash for an arbitrarily chosen preimage, being the number `5` in this example.

First, we create a new file named `hashexample.zara` with the following content:

```zarathustra
{{#include ../../../zarathustra_cli/examples/book/sha256_tutorial/hashexample.zara}}
```

The first line imports the `sha256packed` function from the Zarathustra standard library.

`sha256packed` is a SHA256 implementation that is optimized for the use in the Zarathustra DSL. Here is how it works: We want to pass 512 bits of input to SHA256. However, a `field` value can only hold 254 bits due to the size of the underlying prime field we are using. As a consequence, we use four field elements, each one encoding 128 bits, to represent our input. The four elements are then concatenated in Zarathustra and passed to SHA256. Given that the resulting hash is 256 bit long, we split it in two and return each value as a 128 bit number.

In case you are interested in an example that is fully compliant with existing SHA256 implementations in Python or Solidity, you can have a look at this [blog](https://blog.decentriq.ch/proving-hash-pre-image-zksnarks-zarathustra) post.

Our code is really just using the `sha256packed`, returning the computed hash.

Having our problem described in Zarathustra' DSL, we can now continue using Zarathustra for the rest of our workflow.

First, we compile the program into an arithmetic circuit using the `compile` command.

```
{{#include ../../../zarathustra_cli/examples/book/sha256_tutorial/test.sh:10}}
```

As a next step we can create a witness file using the following command:

```
{{#include ../../../zarathustra_cli/examples/book/sha256_tutorial/test.sh:11}}
```
...
For now, we have seen that we can compute a hash using Zarathustra.
...
First, Victor has to specify what hash he is interested in. Therefore, we have to adjust the zkSNARK circuit, compiled by Zarathustra, such that in addition to computing the digest, it also validates it against the digest of interest, provided by Victor. This leads to the following update for `hashexample.zara`:

```zarathustra
{{#include ../../../zarathustra_cli/examples/book/sha256_tutorial/hashexample_updated.zara}}
```
...
```
{{#include ../../../zarathustra_cli/examples/book/sha256_tutorial/test.sh:15}}
```

Based on that Victor can run the setup phase and export a verifier smart contract as a Solidity file:

```
{{#include ../../../zarathustra_cli/examples/book/sha256_tutorial/test.sh:16:17}}
```
...
```
{{#include ../../../zarathustra_cli/examples/book/sha256_tutorial/test.sh:18}}
```

Finally, Peggy can run the command to construct the proof:

```
{{#include ../../../zarathustra_cli/examples/book/sha256_tutorial/test.sh:19}}
```
...
Zarathustra creates a file, `proof.json`,  consisting of the three elliptic curve points that make up the zkSNARKs proof. The `verifyTx` function in the smart contract deployed by Victor accepts these three values, along with an array of public inputs. The array of public inputs consists of:

* any public inputs to the main function, declared without the `private` keyword
* the return values of the Zarathustra function
...
>
>Zarathustra would welcome ideas to add support for such ceremonies!
