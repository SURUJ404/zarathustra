...
describe("tests", () => {
  ...
  before(() => {
    return initialize().then((defaultProvider) => {
      zarathustraProvider = defaultProvider;
      return fs.promises
  ...
  describe("compilation", () => {
    it("should compile", () => {
      const artifacts = zarathustraProvider.compile(
    ...
    it("should resolve user module", () => {
      ...
      zarathustraProvider.compile(code, options);
    });
  ...
  describe("computation", () => {
    it("should compute with valid inputs", () => {
      ...
      const result = zarathustraProvider.computeWitness(artifacts, ["2"]);
    ...
    it("should throw on invalid input count", () => {
      assert.throws(() => {
        const code = "def main(private field a) -> field { return a * a; }";
        const artifacts = zarathustraProvider.compile(code);
        zarathustraProvider.computeWitness(artifacts, ["1", "2"]);
      });
    });
    ...
    it("should throw on invalid input type", () => {
      assert.throws(() => {
        const code = "def main(private field a) -> field { return a * a; }";
        const artifacts = zarathustraProvider.compile(code);
        zarathustraProvider.computeWitness(artifacts, [true]);
      });
    });
    ...
    it("should log in debug", () => {
      ...
      let logs = [];
      zarathustraProvider.computeWitness(artifacts, [], {
        logCallback: (l) => {
          logs.push(l);
        },
      });
  ...
  const runWithOptions = (options) => {
    ...
    before(() => {
      provider = zarathustraProvider.withOptions(options);
    });
  ...
  const testRunner = (rootPath, testPath, test) => {
    ...
    for (const curve of curves) {
      it(curve, () => {
        let specializedProvider = zarathustraProvider.withOptions({
  ...
  describe("core tests", () => {
    const rootPath = path.resolve("../zarathustra_core_test");
