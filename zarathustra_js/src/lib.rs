...
use zarathustra_abi::{parse_strict, Decode, Encode, Inputs};
use zarathustra_ark::Ark;
use zarathustra_ast::ir;
use zarathustra_ast::ir::ProgEnum;
use zarathustra_ast::typed::abi::Abi;
use zarathustra_ast::typed::types::{ConcreteSignature, ConcreteType, GTupleType};
use zarathustra_bellman::Bellman;
use zarathustra_circom::{write_r1cs, write_witness};
use zarathustra_common::helpers::{BackendParameter, CurveParameter, SchemeParameter};
use zarathustra_common::{CompileConfig, Resolver};
use zarathustra_core::compile::{compile as core_compile, CompilationArtifacts, CompileError};
use zarathustra_core::imports::Error;
use zarathustra_field::{
...
use zarathustra_proof_systems::rng::get_rng_from_entropy;
use zarathustra_proof_systems::{
...
mod internal {
    ...
    pub fn compute<T: Field>(
        ...
        let return_values: serde_json::Value =
            zarathustra_abi::Value::decode(witness.return_values(), *signature.output)
                .into_serde_json();
    ...
    match (backend, scheme) {
        (BackendParameter::Bellman, SchemeParameter::G16) => match prog {
            ...
            ProgEnum::Bls12_381Program(_) => Err(JsValue::from_str(
                "Not supported: https://github.com/Zarathustra/Zarathustra/issues/1200",
            )),
            _ => Err(JsValue::from_str("Not supported")),
        },
